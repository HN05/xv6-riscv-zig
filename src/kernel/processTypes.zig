...
pub var cpuTable: Cpu[param.NCPU] = undefined;
pub var processTable: Process[param.NPROC] = undefined;
pub var initialProcess: *Process = undefined;
pub var nextProcessId: u32 = 1;
pub var processIdLock: lk.SpinLock = .{};
...
pub fn mapKernelStacks(kernelPageTable: ad.PageTablePtr) void {
    for (processTable, 0..) |_, index| {
        ...
        inline for (ml.KSTACK_PAGENUM) |i| {
            ...
            mem.kernelVirtualMap(kernelPageTable, virtualAddress + i * ad.page_size, .fromPtr(page), ad.page_size, .{ .write = true, .read = true });
        }
    }
}
...
pub const Process = struct {
    ...
    state: ProcessState,
    sleepingOnChannel: ?*anyopaque, // If non-null, sleeping on channel
    isKilled: bool,
    exitStatus: u32, // Exit status to be returned to parent's wait
    id: u32, // process id

    // wait_lock must be held when using this:
    parentProcess: ?*Process, // null for root process

    // these are private to the process, so p->lock need not be held.
    kernelStackAddress: ad.UserAddr, // Virtual address of kernel stack
    size: usize, // Size of process memory (bytes)
    pageTable: ad.PageTablePtr, // User page table
    topFreeVirtualPage: ad.UserAddr, // The highest free user virtual mem page. Starts at TRAPFRAME - 2*PGSIZE and goes down as pages are used.
    trapFrame: *TrapFrame, // data page for trampoline.S
    context: Context, // swtch() here to run process
    openFiles: [param.NOFILE]*c.struct_file, // Open files
    currentWorkingDirectory: *c.struct_inode,
    name: [16]u8, // for debugging
    ownedRingbufsCount: usize, // Count of ringbufs owned by this process
};
